 ...
 *
 */
public class BarCode2Pic_Water3 {

	public static void main(String[] args) {
		try {
			 if( args.length < 4) {
				 System.out.println("使用方式: 命令  序号前半部分 序号后半部分 标签总数 保存目录");
				 System.out.println("举例: BarCode2Pic A 999888777 30 d:\\PicWater3");
			 }
			 else {
				 	/**
				 	 * radix = 4:16进制
...
//						String radixStr = Long.toString(serialNo, radix);
						...
						//格式化为长度为6的字符
			            String serialStr = StringUtil.formatString("0", 6, radixStr);
			            
			            //采用10位
			            BitMatrix barMatrix1 = new MultiFormatWriter().encode(serialHeader + serialStr.substring(0, 2),BarcodeFormat.CODE_39, width, height, hints);
			            File outputFile1 = new File(savePath + "\\" + (i+1) + "_1_bar.png"); 
						MatrixToImageWriter.writeToFile(barMatrix1, format, outputFile1); 
			            
			            
			            BitMatrix barMatrix2 = new MultiFormatWriter().encode(serialStr.substring(2),BarcodeFormat.CODE_39, width, height, hints);
