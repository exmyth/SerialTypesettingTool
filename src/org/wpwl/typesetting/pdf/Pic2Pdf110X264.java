 ...
 *
 */
public class Pic2Pdf110X264 {
	public static void main(String[] args) {
		try {
			if( args.length <4) {
				System.out.println("使用方式: 命令  图片目录  PDF目录  图片总数");
				System.out.println("举例: MatrixPdf d:\\outputpic d:\\outputpdf 30 22111110000000001");
				return;
			}
...
//				BaseFont bf = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H",BaseFont.NOT_EMBEDDED);
				...
				for(int i = 1; i <= rowSize ; i++) {
					for(int j = 1; j <=colSize ; j++) {
						...
						absoluteX = (5 + j *(3.5f) + (j - 1)*(24+3.5f+3.5f))*scale;//0.35f是边框线宽度
						absoluteY = ury - (3 + i *(3.5f+24+3.5f + 24) + (i-1)*(0.5f+2+2.5f+6))*scale;
						
						png.scalePercent(24*scale/148*100);//二维码尺寸是148x148,底板是24mm,伸缩百分比就是24*scale/148*100
						...
						x = absoluteX+0.8f*scale + 32;//文字比二维码偏移值是32,这个值是测试出来的
						y = absoluteY-(0.5f+2+2.5f + 2)*scale;
